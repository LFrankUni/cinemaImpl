...
public class MovieShow extends HasIncome implements java.io.Serializable, IMovieShow {
	...
	private MovieShow(Integer id, Movie movie, String start, String end, Boolean threeDimensional, Integer priceInCent,
			Room room, boolean objectOnly) throws PersistenceException, ConstraintViolation {
		super(id, objectOnly);
		...
		if (objectOnly)
			return;
		try {
			movieShowsOfRoomSupervisor.getInstance().add(room, this);
	 ...
	 * to its Cache!
	 */
	public static MovieShow createAlreadyPersistent(MovieShowProxy proxy, Movie movie, String start, String end,
			Boolean threeDimensional, Integer priceInCent, Room room) throws PersistenceException, ConstraintViolation {
	...
	public static MovieShow createFresh(Movie movie, String start, String end, Boolean threeDimensional,
			Integer priceInCent, Room room) throws PersistenceException, ConstraintViolation {
		db.executer.DBDMLExecuter dmlExecuter = PersistenceExecuterFactory.getConfiguredFactory().getDBDMLExecuter();
		Integer id = dmlExecuter.getNextId();
		try {
			dmlExecuter
					.insertInto("HasIncome", "id, typeKey, start, end, threeDimensional, priceInCent",
							id.toString() + ", "
									+ TypeKeyManager.getTheInstance().getTypeKey("CinemaService", "MovieShow")
											.toString()
									+ ", " + "'" + start + "'" + ", " + "'" + end + "'" + ", "
									+ threeDimensional.toString() + ", " + priceInCent.toString());
	...
	public Set<Ticket> getTickets() throws PersistenceException {
		...
		for (ITicket i : movieShowToTicketSupervisor.getInstance().getTickets(this))
	...
	public void setStart(String newStart) throws PersistenceException {
		this.start = newStart;
		try {
			PersistenceExecuterFactory.getConfiguredFactory().getDBDMLExecuter().update("HasIncome", "start",
					"'" + newStart + "'", this.getId());
		} catch (SQLException | NoConnectionException e) {
			throw new PersistenceException(e.getMessage());
		}
	...
	public void setEnd(String newEnd) throws PersistenceException {
		this.end = newEnd;
		try {
			PersistenceExecuterFactory.getConfiguredFactory().getDBDMLExecuter().update("HasIncome", "end",
					"'" + newEnd + "'", this.getId());
		} catch (SQLException | NoConnectionException e) {
			throw new PersistenceException(e.getMessage());
		}
	...
	public void setThreeDimensional(Boolean newThreeDimensional) throws PersistenceException {
		...
		try {
			PersistenceExecuterFactory.getConfiguredFactory().getDBDMLExecuter().update("HasIncome", "threeDimensional",
					newThreeDimensional.toString(), this.getId());
		} catch (SQLException | NoConnectionException e) {
			throw new PersistenceException(e.getMessage());
		}
	}
	...
	public void setPriceInCent(Integer newPriceInCent) throws PersistenceException {
		...
		try {
			PersistenceExecuterFactory.getConfiguredFactory().getDBDMLExecuter().update("HasIncome", "priceInCent",
