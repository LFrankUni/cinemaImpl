...
public class Movie extends HasIncome implements java.io.Serializable, IMovie {
	...
	public static Movie createFresh(String title, String description, Integer minutes) throws PersistenceException {
		...
		try {
			dmlExecuter.insertInto("HasIncome", "id, typeKey, title, description, minutes",
					id.toString() + ", "
							+ TypeKeyManager.getTheInstance().getTypeKey("CinemaService", "Movie").toString() + ", "
							+ "'" + title + "'" + ", " + "'" + description + "'" + ", " + minutes.toString());
	...
	public void setTitle(String newTitle) throws PersistenceException {
		this.title = newTitle;
		try {
			PersistenceExecuterFactory.getConfiguredFactory().getDBDMLExecuter().update("HasIncome", "title",
					"'" + newTitle + "'", this.getId());
		} catch (SQLException | NoConnectionException e) {
			throw new PersistenceException(e.getMessage());
		}
	...
	public void setDescription(String newDescription) throws PersistenceException {
		...
		try {
			PersistenceExecuterFactory.getConfiguredFactory().getDBDMLExecuter().update("HasIncome", "description",
					"'" + newDescription + "'", this.getId());
		} catch (SQLException | NoConnectionException e) {
			throw new PersistenceException(e.getMessage());
		}
	}
	...
	public void setMinutes(Integer newMinutes) throws PersistenceException {
		...
		try {
			PersistenceExecuterFactory.getConfiguredFactory().getDBDMLExecuter().update("HasIncome", "minutes",
					newMinutes.toString(), this.getId());
		} catch (SQLException | NoConnectionException e) {
			throw new PersistenceException(e.getMessage());
		}
	}
