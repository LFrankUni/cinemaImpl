...
public class Ticket extends TicketAction implements java.io.Serializable, ITicket {
	...
	public static Ticket createFresh(Seat seat, MovieShow movieShow, User user)
		...
		try {
			dmlExecuter.insertInto("TicketAction", "id, typeKey", id.toString() + ", "
					+ TypeKeyManager.getTheInstance().getTypeKey("CinemaService", "Ticket").toString());
	...
	public Optional<TicketState> getState() throws PersistenceException {
		...
		return opt.isPresent() ? Optional.of(TicketToStateSupervisor.getInstance().getState(this).get().getTheObject())
				: Optional.empty();
	}

	public void setState(TicketState newState) throws ConstraintViolation, PersistenceException {
		if (this.getState().isPresent())
			TicketToStateSupervisor.getInstance().change(this, this.getState().get(), newState);
		else
			TicketToStateSupervisor.getInstance().set(this, newState);
	}
